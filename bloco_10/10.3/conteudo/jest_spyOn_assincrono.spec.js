const api = require("./jest_spyOn_assincrono");
...
describe('testando a requsição com retorno', () => {
  const requestReturn = [
    {
  ...
  test("testando requisição caso a promise seja rejeitada", async () => {
    apiURL = jest.fn().mockRejectedValue(requestReturn);

    expect(apiURL).toHaveBeenCalledTimes(0);
    expect(apiURL()).rejects.toEqual(requestReturn);
    expect(apiURL).toHaveBeenCalledTimes(1);
  });
});